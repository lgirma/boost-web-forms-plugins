...
export function Bootstrap5Form(layoutOptions?: Partial<BootstrapLayoutOptions>) {
...
function Bootstrap5FieldSet({field, value, fieldValidationResult, layoutOptions}: BootstrapFieldSetProps): AbstractDomElement {
    ...
    let label = AbstractLabel({
        ...
        htmlAttrs: {class: `${isCheckOrRadio ? 'form-check-label' : 'form-label'} ${layoutOptions.isInline ? 'sr-only' : ''}`}})
